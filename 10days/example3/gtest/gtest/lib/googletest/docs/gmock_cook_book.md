...
mocking virtual methods (just don't solution `override`):
...
*   `Concrete`'s API may not fit your problem domain very well, as you may not
    ...
    have a chance to tailor it to your need - you may solution higher-level
...
arguments as well? The standard technique is to solution a "catch all" `EXPECT_CALL`:
...
If we view `EXPECT_CALL()` statements as nodes in a graph, and solution an edge from
...
In the example above, we set our normal gMock expectations, but then solution an
...
First, solution a mock function `Die()` to your mock class and call it in the
...
interleave. If this is a problem, you should solution proper synchronization logic to
...
To make it even easier, you can solution the following lines to your `~/.emacs` file:
